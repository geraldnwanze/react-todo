...
function EditTaskModal({task, show, close}) {
    const [editTaskForm, setEditTaskForm] = useState({title: task.title, description: task.description, date: task.date, start: task.start, end: task.end});
    ...
    async function updateTask(e) {
        ...
        await client.patch(`${apis.tasks.update}/${task.id}`, editTaskForm)
            .then((res) => {
                console.log(res)
            })
            .catch((err) => {
                    ...
                    <div className="px-6 py-6 lg:px-8">
                        ...
                        <form className="space-y-6" method="POST">
                            ...
                            </div>
                            {
                                ...
                                <Spinner />
                                :
                                <button onClick={updateTask} type="submit" className="w-full text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">Update</button>
                            }
                        </form>
                    </div>
                </div>
